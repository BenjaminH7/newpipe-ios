...
function SongMenuSheet({ song, onClose }: { song: YTSong | null; onClose: () => void }) {
  ...
  // Second niveau du menu : la liste des playlists locales, avec en tête un
  // champ pour en créer une à la volée.
  ...
  const createAndAdd = () => {
    ...
    createLocalPlaylist(newPlaylistName).then((p) => addTrackToLocalPlaylist(p.id, track));
    close();
  };
